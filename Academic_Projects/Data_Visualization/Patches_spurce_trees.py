...
low (<2500), medium (2500-3000), high (>3000). the we are taking subset of the dataset where we take only 
those entries where the tree type is spurce, clustering them and coloring them according
...
dataset = dataset[(dataset['Tree'] == 1)]
...
fig = go.Figure(data=[go.Scatter3d(
    ...
    marker=dict(color=Y, colorscale='Rainbow', opacity=0.5),text=[f'elevation: {a}; elevation level:{b}; tree:{c}' 
                                ...
                                hoverinfo='text'
)],layout=go.Layout(title = 'Classification of the Spurce Trees according to their Elevation Level'
                    ,barmode='group'))

offline.plot(fig,filename='spurce_by_elevation.html')
