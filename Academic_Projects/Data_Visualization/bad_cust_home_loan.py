...
'''
Here we are dividing the cutomer into ctageory based on their balance and trying to visualize the bad customers 
and their housing loan allotment
'''
import pandas as pd
import numpy as np
...
def converter(column):
    if column <= 0:
        return 0 # Bad Customers
    elif column > 0 and column <= 2000:
        return 1 # Average Customers
    elif column > 2000 and column <= 5000:
        return 2 # Good Customers
    else:
        return 3 # Very Good Customers

dataset['Customer Category'] = dataset['balance'].apply(converter)

bad_customers = dataset[(dataset['Customer Category'] == 0)]

# Dividing dataset into label and feature sets
X = bad_customers.drop('housing' , axis = 1) # Features
Y = bad_customers['housing'] # Labels

# Normalizing numerical features so that each feature has mean 0 and variance 1
feature_scaler = StandardScaler()
X_scaled = feature_scaler.fit_transform(bad_customers)
...
default= list(bad_customers['default'])
housing = list(bad_customers['housing'])
balance = list(bad_customers['balance'])
customer_category = list(bad_customers['Customer Category'])
data = [go.Scatter3d(x=x_tsne[:,0], y=x_tsne[:,1], z=x_tsne[:,2], mode='markers',
                    marker = dict(color=Y, colorscale='Rainbow', opacity=0.5),
                                text=[f'default: {a}; housing:{b}, balance:{c}, customer category:{d}' 
                                      for a,b,c,d in list(zip(default,housing,balance,customer_category))],
                                hoverinfo='text')]

layout = go.Layout(title = 'Bad Customers Housing Loan Allotment', width = 700, height = 700,
                    xaxis = dict(title='First Dimension'),
                    yaxis = dict(title='Second Dimension'))
fig = go.Figure(data=data, layout=layout)
offline.plot(fig,filename='bad_cust_hsg.html')
