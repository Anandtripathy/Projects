...
'''
Here we are just considering the defaulters and classiying them according to their Age Group
'''
import pandas as pd
import numpy as np
...
defaulters_data = dataset[(dataset['default'] == 1)]

# Dividing dataset into label and feature sets
X = defaulters_data.drop('Age Group' , axis = 1) # Features
Y = defaulters_data['Age Group'] # Labels
...
age_group = list(defaulters_data['Age Group'])
age = list(defaulters_data['age'])
default = list(defaulters_data['default'])
data = [go.Scatter3d(x=x_tsne[:,0], y=x_tsne[:,1], z=x_tsne[:,2], mode='markers',
                    marker = dict(color=Y, colorscale='Rainbow', opacity=0.5),
                                text=[f'age group: {a}; age:{b}, default:{c}' 
                                      for a,b,c in list(zip(age_group,age,default))],
                                hoverinfo='text')]

layout = go.Layout(title = 'Defaulters Age Group', width = 700, height = 700,
                    xaxis = dict(title='First Dimension'),
                    yaxis = dict(title='Second Dimension'))
fig = go.Figure(data=data, layout=layout)
offline.plot(fig,filename='defaulters_age_group.html')
