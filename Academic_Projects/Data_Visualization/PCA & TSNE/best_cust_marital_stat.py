...
'''
here we are creating customer category based on their balance, and identifying the best customers and classifying
them whether they are married or not
'''
import pandas as pd
import numpy as np
...
best_customers = dataset[(dataset['Customer Category'] == 3)]

# Dividing dataset into label and feature sets
X = best_customers.drop('marital' , axis = 1) # Features
Y = best_customers['marital'] # Labels
...
marital = list(best_customers['marital'])
balance = list(best_customers['balance'])
customer_category = list(best_customers['Customer Category'])
data = [go.Scatter3d(x=x_tsne[:,0], y=x_tsne[:,1], z=x_tsne[:,2], mode='markers',
                    marker = dict(color=Y, colorscale='Rainbow', opacity=0.5),
                                text=[f'marital: {a}; balance:{b}, customer category:{c}' 
                                      for a,b,c in list(zip(marital,balance,customer_category))],
                                hoverinfo='text')]

layout = go.Layout(title = 'Best Customers Marital Status', width = 700, height = 700,
                    xaxis = dict(title='First Dimension'),
                    yaxis = dict(title='Second Dimension'))
fig = go.Figure(data=data, layout=layout)
offline.plot(fig,filename='best_cust_martital_sts.html')
