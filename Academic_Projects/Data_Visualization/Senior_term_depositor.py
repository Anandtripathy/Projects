...
'''
here we are trying displaying the customers who are aged above 50 and have term deposit or not
'''
import pandas as pd
import numpy as np
...
senior_citizens_data = dataset[(dataset['age'] > 50)]

# Dividing dataset into label and feature sets
X = senior_citizens_data.drop('term' , axis = 1) # Features
Y = senior_citizens_data['term'] # Labels

# Normalizing numerical features so that each feature has mean 0 and variance 1
feature_scaler = StandardScaler()
X_scaled = feature_scaler.fit_transform(senior_citizens_data)
...
age = list(senior_citizens_data['age'])
job = list(senior_citizens_data['job'])
default = list(senior_citizens_data['default'])
term = list(senior_citizens_data['term'])
data = [go.Scatter3d(x=x_tsne[:,0], y=x_tsne[:,1], z=x_tsne[:,2], mode='markers',
                    marker = dict(color=Y, colorscale='Rainbow', opacity=0.5),
                                text=[f'age: {a}; job:{b}, default:{c}, term:{d}' 
                                      for a,b,c,d in list(zip(age,job,default,term))],
                                hoverinfo='text')]

layout = go.Layout(title = 'Senior Customers with/without Term Deposits', width = 700, height = 700,
                    xaxis = dict(title='First Dimension'),
                    yaxis = dict(title='Second Dimension'))
fig = go.Figure(data=data, layout=layout)
offline.plot(fig,filename='senior_term_depositors.html')
